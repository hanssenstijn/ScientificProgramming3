...
```{r}
#result = dplyr::bind_rows(lapply(parse.smiles(unlist(smiles)),eval.desc,descNames))
```
...
# parLapplyResult = dplyr::bind_rows(parLapply(cl,parse.smiles(unlist(smiles)),eval.desc,descNames))
```

```{r}
#parallelResult = dplyr::bind_rows(mclapply(parse.smiles(unlist(smiles)),eval.desc,descNames,mc.cores = 1))
```
...
foreachResults = foreach(i=1:n,.combine=rbind) %dopar% eval.desc(parse.smiles(smiles[[i]]),descNames,verbose=F)    
